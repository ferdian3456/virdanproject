...
func (repository *Repository) InsertConversationIdempotent(ctx context.Context, tx pgx.Tx, c DmConversation) (bool, error) {
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("insert conversation failed", zap.Error(err))
		return false, err
	}
	return tag.RowsAffected() == 1, nil
}

func (repository *Repository) GetConversationByPair(ctx context.Context, tx pgx.Tx, serverId, low, high string) (DmConversation, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("get conversation by pair failed", zap.Error(err))
		return DmConversation{}, err
	}
	return conversation, nil
}

func (repository *Repository) InsertConversationStateIdempotent(ctx context.Context, tx pgx.Tx, conversationId, userId, serverId, peerUserId string, now time.Time) error {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("insert conversation state failed", zap.Error(err))
		return err
	}
	return nil
}

func (repository *Repository) GetConversationById(ctx context.Context, conversationId string) (DmConversation, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return DmConversation{}, pgx.ErrNoRows
	}
	if err != nil {
		return DmConversation{}, err
	}
...
func (repository *Repository) InsertMessageIdempotent(ctx context.Context, tx pgx.Tx, m DmMessage) (bool, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("insert message failed", zap.Error(err))
		return false, err
	}
	return tag.RowsAffected() == 1, nil
}

func (repository *Repository) GetMessageByClientId(ctx context.Context, tx pgx.Tx, conversationId, senderId, clientMessageId string) (DmMessage, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("get message by client id failed", zap.Error(err))
		return DmMessage{}, err
	}
	return message, nil
}

func (repository *Repository) UpdateConversationLastMessage(ctx context.Context, tx pgx.Tx, conversationId, updatedBy string, at time.Time) error {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("update conversation last message failed", zap.Error(err))
	}
	return err
}

func (repository *Repository) BumpConversationStates(ctx context.Context, tx pgx.Tx, conversationId, senderId, preview string, at time.Time) error {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("bump conversation states failed", zap.Error(err))
	}
	return err
}

func (repository *Repository) ListMessages(ctx context.Context, conversationId, serverId string, limit int, cursor *DmMessageCursor, minioFullUrl string) ([]DmMessageResponse, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	out := make([]DmMessageResponse, 0)
	for rows.Next() {
		var messageRow DmMessageResponse
...
func (repository *Repository) ListConversations(ctx context.Context, userId, serverId string, limit int, cursor *DmConversationCursor, minioFullUrl string) ([]DmConversationResponse, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	out := make([]DmConversationResponse, 0)
	for rows.Next() {
		var conv DmConversationResponse
		var objectKey *string
		if err = rows.Scan(
...
func (repository *Repository) ListMembers(ctx context.Context, callerId, serverId, q string, limit int, cursor *DmMemberCursor, minioFullUrl string) ([]DmMemberResponse, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	out := make([]DmMemberResponse, 0)
	for rows.Next() {
		var member DmMemberResponse
		var objectKey *string
		if err = rows.Scan(
...
func (repository *Repository) GetMessageCreatedAt(ctx context.Context, conversationId, messageId string) (time.Time, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
...
func (repository *Repository) MarkRead(ctx context.Context, conversationId, userId string, lastReadMessageId *string, readAt, now time.Time) error {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		shared.GetLoggerWithTraceContext(ctx, repository.Log).Error("mark read failed", zap.Error(err))
	}
	return err
}

func (repository *Repository) GetMemberIdentity(ctx context.Context, serverId, userId, minioFullUrl string) (DmIdentity, error) {
	serviceName := repository.Config.String("OTEL_SERVICE_NAME")
	...
	if err != nil {
		return DmIdentity{}, err
	}
	identity.AvatarUrl = chatAvatarUrl(minioFullUrl, objectKey)
	return identity, nil
}

func (repository *Repository) GetConversationPeerIds(ctx context.Context, userId string) ([]string, error) {
