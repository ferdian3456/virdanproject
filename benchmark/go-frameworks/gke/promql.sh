...
if [ $# -ge 4 ]; then
  curl -sS --retry 3 --retry-all-errors -H "Authorization: Bearer $token" "$base/query_range" \
    --data-urlencode "query=$1" -d "start=$2" -d "end=$3" -d "step=$4"
else
  curl -sS --retry 3 --retry-all-errors -H "Authorization: Bearer $token" "$base/query" --data-urlencode "query=$1"
fi
